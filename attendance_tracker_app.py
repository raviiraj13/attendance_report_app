...
if uploaded_file:
    ...
    plt.figure(figsize=(6,6))
    plt.pie(values, labels=['Present','Absent'], autopct='%1.1f%%', startangle=90,
    ...
    # Target Aggregate Attendance (all subjects)
    target_aggregate = st.number_input("Enter target aggregate attendance (%)", min_value=0, max_value=100, value=75, step=1)
    aggregate_classes_needed = target_attendance(total_present, total_classes, target_aggregate)
    st.write(f"🎯 You need to attend **{aggregate_classes_needed} more classes overall** to reach {target_aggregate}% aggregate attendance.")

    # Pie chart for selected subject
